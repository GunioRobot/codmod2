...
results = np.lib.recfunctions.append_fields(data, 'prediction', predicted_y)
lower_y = mod_mc.param_pred.stats()['95% HPD interval'][:,0]
results = np.lib.recfunctions.append_fields(results, 'lower', lower_y)
upper_y = mod_mc.param_pred.stats()['95% HPD interval'][:,1]
results = np.lib.recfunctions.append_fields(results, 'upper', upper_y)
